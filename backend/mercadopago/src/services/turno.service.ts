...
export async function markTurnoAsPaid(params: {
  ...
  await db.runTransaction(async tx => {
    ...
    if (!snap.exists) {
      console.error('[Turno] Documento no existe:', turnoId);
      return;
    }
    ...
    const origenPortal = d['origenCreacion'] === 'portal';
    const estadoActual = d['estado'] as string | undefined;

    if (!origenPortal && estadoActual === 'pendiente') {
...
export async function updateTurnoWithPreference(params: {
  turnoId: string;
  preferenceId: string;
  externalReference: string;
